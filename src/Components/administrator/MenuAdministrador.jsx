...
function MenuAdministrador() {
  ...
  useEffect(() => {
    const fetchTickets = async () => {
      try {
        const response = await axios.get('http://localhost:5053/api/Administrador/ListaConteoTickets');
    ...
    <div className="dashboard-wrapper">
      ...
      <div className="main-content">
        ...
        <div className="navbar">
          <h5>Dashboard</h5>
          <div>
            <button className="btn btn-outline-light btn-sm"><i className="fas fa-bell"></i></button>
            <button className="btn btn-outline-light btn-sm"><i className="fas fa-user-circle"></i></button>
          </div>
        </div>
                  ...
                  <tbody>
                    {tickets.map(ticket => (
                      <tr key={ticket.id}>
                        ...
                        <td>{ticket.titulo}</td>
                        <td>
                          <span className={`badge ${getBadgeClass(ticket.estado)}`}>
                            {ticket.estado}
                          </span>
                        </td>
